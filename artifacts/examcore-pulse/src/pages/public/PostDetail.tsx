  ...
  return (
    <PublicLayout>
      <article className="max-w-4xl mx-auto px-4 sm:px-6 py-8 md:py-16">
        ...
        {/* Interaction Bar */}
        <div className="py-6 border-y border-border mb-16 space-y-3">
          ...
          {post.images && post.images.length > 0 && (post as any).pdfDownloadEnabled !== false && (
            <Button
              variant="outline"
              size="lg"
              className="w-full rounded-full gap-2 border-primary/40 text-primary hover:bg-primary hover:text-primary-foreground transition-colors"
              onClick={handleDownloadPDF}
              disabled={downloading}
            >
              <Download className="w-5 h-5" />
              {downloading ? "Preparing PDF…" : ((post as any).pdfButtonLabel || "Download Timetable PDF")}
            </Button>
          )}
        </div>
