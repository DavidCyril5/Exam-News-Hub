...
export default function PostEditor() {
  ...
  const [pdfButtonLabel, setPdfButtonLabel] = useState("Download Timetable PDF");
  ...
  useEffect(() => {
    if (isEditing && existingPost) {
      ...
      setPdfDownloadEnabled((existingPost as any).pdfDownloadEnabled !== false);
      setPdfButtonLabel((existingPost as any).pdfButtonLabel || "Download Timetable PDF");
    }
  ...
  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    const payload = { title, categoryId, excerpt, content, coverImage, status, images, pdfDownloadEnabled, pdfButtonLabel };
    const options = {
      onSuccess: () => {
          ...
          <div className="space-y-6">
            ...
            <Card className="p-4 sm:p-6 space-y-4">
              <div className="flex items-center gap-2 mb-1">
                ...
                <label className="text-sm font-bold">PDF Download Button</label>
              </div>
              ...
              <div className="flex items-center justify-between py-2 px-3 rounded-lg bg-muted/40">
                ...
                <button
                  ...
                  onClick={() => setPdfDownloadEnabled(v => !v)}
                  className={`relative inline-flex h-6 w-11 shrink-0 cursor-pointer rounded-full border-2 border-transparent transition-colors focus:outline-none focus:ring-2 focus:ring-primary focus:ring-offset-2 ${
                    pdfDownloadEnabled ? "bg-primary" : "bg-muted-foreground/30"
                  }`}
                >
                  <span
                    className={`pointer-events-none inline-block h-5 w-5 transform rounded-full bg-white shadow ring-0 transition-transform ${
                      pdfDownloadEnabled ? "translate-x-5" : "translate-x-0"
                    }`}
                  />
                </button>
              </div>
              ...
              {pdfDownloadEnabled && (
                <div>
                  <label className="text-xs font-medium text-muted-foreground mb-1.5 block">Button label</label>
                  <Input
                    value={pdfButtonLabel}
                    onChange={e => setPdfButtonLabel(e.target.value)}
                    placeholder="Download Timetable PDF"
                    className="text-sm"
                  />
                  <p className="text-xs text-muted-foreground mt-1.5">This is the text shown on the button.</p>
                </div>
              )}

              {!pdfDownloadEnabled && (
                <p className="text-xs text-muted-foreground">The download button will be hidden from readers.</p>
              )}
            </Card>
